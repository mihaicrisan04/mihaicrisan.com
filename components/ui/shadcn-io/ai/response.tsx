...
const DOUBLE_ASTERISK_GLOBAL = /\*\*/g;
const DOUBLE_UNDERSCORE_GLOBAL = /__/g;
const TRIPLE_BACKTICK_GLOBAL = /```/g;
const CODE_BLOCK_GLOBAL = /```[\s\S]*?```/g;
const DOUBLE_TILDE_GLOBAL = /~~/g;
...
function parseIncompleteMarkdown(text: string): string {
	...
	if (boldMatch) {
		...
		const asteriskPairs = (result.match(DOUBLE_ASTERISK_GLOBAL) || []).length;
	...
	if (italicMatch) {
		...
		const underscorePairs = (result.match(DOUBLE_UNDERSCORE_GLOBAL) || [])
			.length;
	...
	if (inlineCodeMatch) {
		...
		const _completeCodeBlocks = (result.match(CODE_BLOCK_GLOBAL) || []).length;
		const allTripleBackticks = (result.match(TRIPLE_BACKTICK_GLOBAL) || [])
			.length;
	...
	if (strikethroughMatch) {
		// Count the number of ~~ in the entire string
		const tildePairs = (result.match(DOUBLE_TILDE_GLOBAL) || []).length;
...
const components: Options["components"] = {
	...
	pre: ({ node, className, children }) => {
		...
		if (isValidElement(children) && children.props) {
			const childProps = children.props as CodeChildrenProps;
